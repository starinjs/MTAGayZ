...
function showDayZDeathScreen()
	...
	deathText = guiCreateLabel(0,0.8,1,0.2,"You died! \nYou will respawn in "..gameplayVariables["playerRespawnCountdown"].." seconds.",true)
	guiLabelSetHorizontalAlign (deathText,"center")
	setTimer(function() guiSetVisible(deathText,false) end,gameplayVariables["playerRespawnCountdown"] * 1000,1)
	setTimer(function() guiSetVisible(deadBackground,false) end,gameplayVariables["playerRespawnCountdown"] * 1000,1)
	setTimer(destroyElement,gameplayVariables["playerRespawnCountdown"] * 1000,1,deathText)
	setTimer(destroyElement,gameplayVariables["playerRespawnCountdown"] * 1000,1,deadBackground)
	local countdown = gameplayVariables["playerRespawnCountdown"]
	setTimer(function() countdown = countdown-1 guiSetText(deathText,"You died! \nYou will respawn in "..countdown.." seconds.") end,1000,gameplayVariables["playerRespawnCountdown"]-1)
end